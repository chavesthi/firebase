...
import { doc, getDoc, updateDoc, deleteDoc as deleteFirestoreDoc, serverTimestamp } from 'firebase/firestore';
...
const UserProfilePage: NextPage = () => {
  ...
  const [currentPhotoURL, setCurrentPhotoURL] = useState<string | null>(null); // Stores the URL of the currently saved photo
  const [editorFile, setEditorFile] = useState<File | null>(null); // Stores the file selected by the user for the editor
  ...
  const handleImageChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.files && event.target.files[0]) {
      ...
      setEditorFile(file);
      setEditorScale(1.2); // Reset scale on new image
      setEditorRotation(0); // Reset rotation
    }
  };
  ...
  const saveProfileChanges = async (data: UserProfileFormInputs) => {
    ...
    if (editorFile && editorRef.current) {
      ...
      const blob = await new Promise<Blob | null>((resolve) => canvas.toBlob(resolve, editorFile.type, 0.90)); // Compress a bit
      ...
      const imagePath = `fotosperfilusuario/${currentUser.uid}/${Date.now()}_${croppedImageFile.name}`; // Add timestamp to ensure uniqueness
    ...
    // Proceed to save all data to Firestore
    try {
      ...
      setCurrentPhotoURL(finalPhotoURL); // Update local state for display
      setEditorFile(null); // Clear editor state
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      ...
      await deleteFirestoreDoc(doc(firestore, "users", currentUser.uid)); 
      await deleteUser(currentUser);
      toast({ title: "Conta Excluída", variant: "default", duration: 7000 });
      router.push('/login'); 
    } catch (error: any) {
        ...
        <form onSubmit={handleSubmit(saveProfileChanges)}>
          <CardContent className="space-y-6 p-4 sm:p-6">
              ...
              <div className="w-32 h-32 sm:w-40 sm:h-40 rounded-full border-2 border-primary bg-muted flex items-center justify-center overflow-hidden">
                {editorFile ? (
                  <AvatarEditor
                    ref={editorRef}
                    image={editorFile}
                    width={200} // Internal canvas width
                    height={200} // Internal canvas height
                    border={25} // Border inside the canvas, effectively padding
                    borderRadius={125} // Makes the cropping area circular
                    color={[0, 0, 0, 0.6]} // RGBA background color
                    scale={editorScale}
            ...
            <div className="space-y-2">
              ...
              <Controller name="age" control={control} render={({ field: { onChange, onBlur, value, name, ref } }) => <Input id="age" type="number" placeholder="Sua idade" name={name} ref={ref} value={value ?? ''} onChange={e => onChange(val === '' ? undefined : parseInt(val, 10))} onBlur={onBlur} className={errors.age ? 'border-destructive' : ''} />} />
            ...
            <div className="space-y-2">
              ...
              <ScrollArea className="h-32 p-2 border rounded-md border-input"><div className="grid grid-cols-1 gap-2 sm:grid-cols-2">{MUSIC_STYLE_OPTIONS.map((option) => (<div key={option.value} className="flex items-center space-x-2"><Controller name="preferredMusicStyles" control={control} render={({ field }) => (<Checkbox id={`music-${option.value}`} checked={field.value?.includes(option.value)} onCheckedChange={(checked) => {const currentSelection = field.value || []; if (checked) {if (currentSelection.length < 4) {field.onChange([...currentSelection, option.value]);} else {toast({ title: "Limite atingido", variant: "destructive", duration: 3000 }); return false;}} else {field.onChange(currentSelection.filter((value) => value !== option.value));} return checked;}} disabled={!field.value?.includes(option.value) && (field.value?.length ?? 0) >= 4} />)} /><Label htmlFor={`music-${option.value}`} className="font-normal text-foreground/80 text-xs">{option.label}</Label></div>))}</div></ScrollArea>
          ...
          <CardFooter className="flex flex-col gap-4 p-4 sm:p-6">
            ...
            <div className="w-full space-y-2">
                ...
                <AlertDialog open={showDeleteDialog} onOpenChange={(open) => { if (!open) { setDeletePassword(''); setShowPasswordInput(false); } setShowDeleteDialog(open); }}>
                    ...
                    <AlertDialogContent>
                        <AlertDialogHeader><AlertDialogTitle className="text-destructive">Excluir Conta Permanentemente?</AlertDialogTitle><AlertDialogDescription>Esta ação é irreversível. Todos os seus dados serão removidos. Para continuar, insira sua senha.</AlertDialogDescription></AlertDialogHeader>
